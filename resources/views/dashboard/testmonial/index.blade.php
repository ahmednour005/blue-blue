                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$loop->iteration}}</td>
                                 ...
                                 <td><img src="{{ asset('uploads/'.$testmonial->image) }} " height="100px" width="100px" alt=""></td>

                                <td>
                                    <form action="{{route('testmonial.destroy',$testmonial->id)}}" method="POST" class="remove-record-model">
                                        @csrf
                                        @method('delete')
                                    <button class="btn btn-danger deleteExpense" ><i class="mdi mdi-delete"></i></button>

                                    </form>
                                     <a type="button" href="{{ route('testmonial.edit',$testmonial->id) }}" class="btn btn-success waves-effect waves-light " ><i class="mdi mdi-pencil-outline"></i></a>
                                </td>
                            </tr>
                            @endforeach
...
{{-- <div id="applicantDeleteModal" class="modal modal-danger fade" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" style="width:55%;">
        <div class="modal-content">
            <form action="{{route('testmonial.destroy')}}" method="POST" class="remove-record-model">
                @csrf
                @method('delete')
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title text-center" id="custom-width-modalLabel"> تاكيد حذف  العميل</h4>
                </div>
