...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">    اللغات</h4>
                    ...
                    <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                        ...
                        <tbody>
                            @foreach($category_translations as $category)
                            <tr>
                                <td>{{$loop->iteration}}</td>
                                 <td>{{$category->title_ar}}</td>
                                 <td>{{$category->title_en}}</td>
                                <td>
                                    <button class="btn btn-danger deleteExpense" data-userid="{{$category->id}}"><i class="mdi mdi-delete"></i></button> 
                                     <a type="button" href="{{ route('category-translation.edit',$category->id) }}" class="btn btn-success waves-effect waves-light " ><i class="mdi mdi-pencil-outline"></i></a>
                                </td>
                            </tr>
                            @endforeach
...
<div id="applicantDeleteModal" class="modal modal-danger fade" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" style="width:55%;">
        <div class="modal-content">
            <form action="{{route('category.translation.delete')}}" method="POST" class="remove-record-model">
                @csrf
                @method('delete')
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title text-center" id="custom-width-modalLabel"> تاكيد الحذف  </h4>
                </div>
                <div class="modal-body">
                    <h4>هل أنت متأكد من الحذف</h4>
                    <input type="hidden" , name="category_id" id="app_id">
                </div>
...
<script src="{{asset('dashboard/assets/libs/datatables.net-buttons/js/buttons.translation.min.js')}}"></script>
